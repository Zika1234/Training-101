...
exports.readUserId = function (id, x) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, (err, dbvar) => {
        ...
        dbase.collection('userDetails').find({ 'emailId': id }).toArray((err, result) => {
...
exports.newPass = function (emailId, newPassword, x) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        var dbobj = db.db("userManagement");
        var query = { 'emailId': emailId };
        dbobj.collection("loginCredentials").updateOne(query, { $set: { "password": encrypt(newPassword) } }, true, function (err, result) {
            if (err) throw err;
            console.log(result);
            x(result);
            db.close();
...
module.exports.addUserDetails = function (data) {
    return MongoClient.connect(mongourl, { useNewUrlParser: true }).then((dbobj) => {
        // if (err) throw err;
        console.log(data)
        var db = dbobj.db('userManagement');
        var uid = uuidv4()
        db.collection('loginCredentials').insertOne({ "userId": uid, "emailId": data.emailId, "password": encrypt(data.password), "quesId": data.securityQ, "answer": data.answer }, true)
        // , (err, res) => {
        //     if (err) throw err;
        //     console.log(res);
        // })

        return db.collection('userDetails').insertOne({ "userId": uid, "myAds": [], "subscriptions": [], "mailBox": [], "firstName": data.firstName, "lastName": data.lastName, "phoneNumber": data.phoneNumber, "address": { "city": data.city, "landmark": data.landmark, "pincode": data.pincode }, "emailId": data.emailId, "dateOfBirth": data.dateOfBirth }, true)
        //(err, res) => {
        //     if (err) throw err;
        //     console.log(res);
...
var dbConnection = MongoClient.connect(mongourl, { useNewUrlParser: true });
...
module.exports.isPhoneAvailable1 = function (phone) {
    return dbConnection
        .then(dbobj => {
            ...
            var db = dbobj.db('userManagement');
            return db.collection('userDetails').find({ phoneNumber: phone }).count()
        }).then(count => { return count }).catch(err => console.log(err));
...
module.exports.isEmailAvailable1 = (phone) => {
    return dbConnection
        .then(dbobj => {
            ...
            var db = dbobj.db('userManagement');
            console.log(phone)
            return db.collection('userDetails').find({ emailId: phone }).count()
            // if(cnt ===0){
            //     console.log(cnt)
            //     return true;
            // }else{
